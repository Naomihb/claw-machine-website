...
export function ProductDetail({ product }: { product: Product }) {
  ...
  const t = dictionary[language].productPage

  const bestFor = language === 'zh' ? product.bestForZh : product.bestFor
  const description =
    ...
    <main className="flex-1">
              ...
              {product.gallery.map((photo) => (
                <figure key={photo.src} className="flex flex-col">
                  ...
                  <figcaption className="mt-3 text-sm leading-relaxed text-muted-foreground">
                    {language === 'zh' ? photo.captionZh : photo.caption}
                  </figcaption>
                </figure>
              ))}
            </div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-6xl px-4 py-16 text-center sm:px-6 lg:px-8 lg:py-24">
          <h2 className="text-balance font-heading text-3xl font-bold tracking-tight sm:text-4xl">
            {t.interestedInPrefix} {product.name}
            {t.interestedInSuffix}
          </h2>
